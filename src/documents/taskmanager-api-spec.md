# Техническое задание - TaskManager API
...
### Маппинг методов и эндпоинтов

#### Методы TaskManager:
- `getTasks()`
- `getTaskById()`
- `addTask(task, updateTask(task))`
- `deleteTaskById()`
- `deleteAllTasks()`
- `subtask methods`
- `epic methods`
...
Добавьте в него реализацию `FileBackedTaskManager`, которую можно получить из утилитного класса `Managers`. После этого можно реализовать маппинг запросов на методы интерфейса `TaskManager`.

API должен работать так, чтобы все запросы по пути `/tasks/<ресурс>` приходили в интерфейс `TaskManager`. Путь для обычных задач — `/tasks/task`, для подзадач — `/tasks/subtask`, для эпиков — `/tasks/epic`. Получить все задачи сразу можно будет по пути `/tasks/`, а получить историю задач по пути `/tasks/history`.
...
Проверять API можно несколькими способами:

1. **Через Insomnia**
2. **С помощью плагина для браузера**, к примеру, RESTED, Postman, RESTClient или других. Выбрать и скачать подходящий можно по [ссылке](ссылка).
...
3. **`GET /load/<ключ>?API_TOKEN=`** — возвращать сохранённое значение по ключу.
...
Теперь можно создать новую реализацию интерфейса `TaskManager` — класс `HttpTaskManager`. Он будет наследовать от `FileBackedTaskManager`.
...
Теперь можно добавить тесты для `HttpTaskManager` аналогично тому как сделали для `FileBackedTaskManager`, отличие только, вместо проверки восстановления состояния менеджера из файла, данные будут восстанавливаться с `KVServer` сервера.
